...
// This variables will be used to assure that the player, will be congratulated only one time adter reaching 2048, 4096, 8192
...
function setGame(){
    ...
    board = [
        ...
        [0, 0, 0, 0],
        [0, 0, 0, 0]
    ]; // Goal, we will use this backend baoard to create our frontend board.
...
// updateTile() - updates the apperance of the tile (that is should have tile number and background color)
...
function slideLeft(){
	for(let r=0; r<rows; r++){
		...
		for(let c = 0; c<columns; c++){
            ...
            // this code is to retrieve the tile element
            ...
            // Updates the apperance of the tile
			updateTile(tile, num);
		}
	}
}
...
function slide(row){
	...
	for(let i = 0; i<row.length - 1; i++){
		...
		if(row[i] == row[i+1]){
			row[i] *= 2; // 4 2 (times 2)
			row[i+1] = 0; // 4 0 (previous tile becomes 0 / becomes blank no value)
            score += row[i];
		}
	}
    ...
    for(let r=0; r < rows; r++){
        for(let c=0; c < columns; c++){
            ...
            if(board[r][c] == 2048 && is2048Exist == false){
                ...
                alert("Victory! You have reached at 8192! You are incredibly awesome!");
                is8192Exist = true;
            }
        }
    }
